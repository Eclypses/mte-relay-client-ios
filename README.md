...
- **MTE core 4.2.1** (via `mte-client-ios`) - Provided automatically as a Swift Package dependency; no separate download is required. SPM resolves it when you add `MteRelay`
...
The MteRelay server acts as a secure intermediary — it receives encrypted requests from your iOS app, decrypts them, and forwards them to your backend API endpoint.
...
class YourNetworkManager {
    ...
    func instantiateMteRelay() async throws {
        // Creates the Relay and validates the MTE license.
        // Pairing with the server happens automatically on the first request.
...
struct ContentView: View {
    @State private var networkManager: YourNetworkManager?
...
class MyViewController: UIViewController {
    var networkManager: YourNetworkManager?
...
Once you've set up your `Relay` instance, use it to make secure requests. The library automatically encrypts your data, sends it through the MteRelay server, and decrypts the response.
...
- **`headersToEncrypt`** (`[String]?`) - Array of header names to encrypt. Pass `nil` or `[]` to send all headers unencrypted
...
class FileUploadManager: RelayStreamDelegate,
    ...
    func uploadStreamedFile(at fileURL: URL) async throws {
        ...
        // Content-Length must be the exact total unencrypted byte count.
        let contentLength = helper.calculateContentLength()
        request.setValue("\(contentLength)", forHTTPHeaderField: "Content-Length")

        let (_, _) = try await relay.uploadFileStream(
            request: request,
            headersToEncrypt: nil,
            pathnamePrefix: nil
        )
    }
    ...
    // Called by the Relay to request bytes. Write the complete request body
    // to outputStream, then close it.
...
class MultipartHelper {
    ...
    func calculateContentLength() -> Int {
        var length = getMultipartPrefix().count + getMultipartPostfix().count
    ...
    func getMultipartPrefix() -> [UInt8] {
        var buf = [UInt8]()
        buf.append(contentsOf: "--\(boundary)\r\n".utf8)
        buf.append(contentsOf: "Content-Disposition: form-data; name=\"file\"; filename=\"\(filename)\"\r\n".utf8)
        buf.append(contentsOf: "Content-Type: application/octet-stream\r\n\r\n".utf8)
        return buf
    }
    ...
    func assembleMultipartWithFile(outputStream: OutputStream) -> Int {
        ...
        do {
            ...
            total += write(Data(getMultipartPrefix()), to: outputStream)

            let chunkSize = 1024 * 1024
            var chunk: Data
            repeat {
                chunk = handle.readData(ofLength: chunkSize)
                if !chunk.isEmpty { total += write(chunk, to: outputStream) }
            } while !chunk.isEmpty

            total += write(Data(getMultipartPostfix()), to: outputStream)
            outputStream.close()
        } catch {
    ...
    private func write(_ data: Data, to stream: OutputStream) -> Int {
        var remaining = data.count
        var written = 0
        while remaining > 0 {
            let n = data.subdata(in: written..<written + remaining).withUnsafeBytes {
                stream.write($0.bindMemory(to: UInt8.self).baseAddress!, maxLength: remaining)
            }
...
Task {
    do {
        ...
        try await uploadManager.uploadStreamedFile(at: fileURL)
    } catch {
...
When your app first connects to the MteRelay server, it establishes MTE pairs automatically. Sometimes you need to re-establish these pairs, for example after a server restart or when you detect persistent encryption failures.
...
| `autoRetryAfterDecodeFailure` | `false` | — | Opt-in: auto-retry once on token-mismatch decode failure |
...
do {
    try await relay.adjustRelaySettings(
        ...
        settings: RelayHostSettings(
            ...
            autoRetryAfterDecodeFailure: false
        )
    )
...
**Common log entries:**
```
"Using iOS Relay Version 5.0.0 and MTE Version 4.2.1"  → Relay initialized
...
The MteRelay library uses delegates for streaming-specific callbacks. Non-streaming requests and control operations are async/throws and do not require delegates.
...
Solutions:
1. Verify you have a valid MTE license from Eclypses
2. Contact [info@eclypses.com](mailto:info@eclypses.com) to verify your license is active
3. Ensure your bundle identifier matches your license
...
#### `dataTask(with:headersToEncrypt:pathnamePrefix:) async throws -> (Data, URLResponse)`
Makes an encrypted HTTP request.
- **Parameters:**
  - `with`: URLRequest to send (URL scheme+host+port is used to locate the relay host)
  - `headersToEncrypt`: Header names to encrypt; `nil` encrypts none
...
The `Classes/MteRelay/` directory keeps the public interface clearly separated from implementation:
