...
```swift
// 1. Add the package — product `Relay`, from 5.2.1.       → Installation
import Relay
...
- **A code-signed app** - Relay stores a small host record (the relay's URL and your client id) in the keychain, so the app must be code-signed. Ad-hoc signing is enough and **no entitlements file is required**. Xcode does this by default; a generated or CI project that disables signing will build fine and then fail on the *first request* with a keychain error. In XcodeGen that means:
  ...
  Pair state itself is *not* persisted
...
3. Choose **Up to Next Major Version**, starting from `5.2.1`
4. Click **Add Package**
...
// Package.swift
dependencies: [
    .package(url: "https://github.com/Eclypses/mte-relay-client-ios.git", from: "5.2.1")
],
targets: [
    .target(
        name: "YourTarget",
...
- **On first use, as above.** The `async` is absorbed by a function that was
  already `async`, so callers and views are untouched. Recommended. Note this
  requires a **reference type** — caching the Relay means mutating stored state,
  which a `struct` cannot do from a non-`mutating` async method. If your network
  layer is currently a struct, make it a `final class`; that is usually the only
  structural change the integration asks for.
...
**Common log entries:**
```
"Using iOS Relay Version 5.2.1 and MTE Version 4.2.1"  → Relay initialized
...
1. Don't encrypt headers that don't exist in the request:
   ```swift
   // Harmless but pointless: naming a header the request does not set.
   // Only headers actually present are considered, so this is a no-op —
   // it does not fail, and it is not a cause of 500s.
   let headersToEncrypt = ["Content-Type"]

   // Clearer: name only headers you actually set. `Content-Type` is
   // encrypted automatically whenever it is present, listed or not.
...
4. **Check MTE version:** `MteBase` comes from the `Mte` module, which arrives as
   a transitive dependency — you must import it explicitly to use it directly.
   ```swift
   import Mte
   print("MTE Version: \(MteBase.getVersion())")
   ```
   Relay also logs both versions at startup, so you can read them from the log
