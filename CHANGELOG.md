...
- Added `RelayHostSettings` struct — per-host configuration value type replacing individual settings parameters. Fields: `streamChunkSize`, `minPairs`, `basePairs`, `maxPairs`, `keepAliveInterval`, `acquisitionWaitTime`, `autoRetryAfterDecodeFailure`.
...
- Added `autoRetryAfterDecodeFailure` to `RelayHostSettings` (default `false`) — opt-in auto-retry after token-mismatch decode failure.
