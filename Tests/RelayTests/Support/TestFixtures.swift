...
enum TestFixtures {
    ...
    /// A generic error that is neither `RelayClientError` nor `RelayError`, so it exercises
    ...
    // Legacy RelayError variants (bridged via RelayClientError.from(_:))
    static let fixtureErrors: [Error] = [
        RelayError.networkError,
        RelayError.mteDecodeError,
